...
def process_rvtools_file(input_filepath, output_folder, original_basename):
    ...
    at the beginning, and applies filters.
    """
    ...
    # --- Categorization Helper Functions --- (Keep these as defined before)
    def assign_workload_category(row): # ... (logic including General DB, Multi-Error) ...
        ...
        if yes_count > 1: return "Multi-Error"
            ...
            for workload, is_yes in is_values.items():
                if is_yes: return workload
        else: # yes_count is 0
            if any(db_check_flags.values()): return "General DB"
    ...
    try:
        ...
        destfile.save(output_filepath); destfile.close(); destfile = None
        ...
        try: trunc_cols_vPart(output_filepath)
        ...
        # --- Step 11: Openpyxl Final Cleanup (NO filtering here anymore) ---
        yield sse_message("Step 11: Reloading for final openpyxl cleanup..."); time.sleep(0.1)
        destfile = xl.load_workbook(output_filepath)
        removeFormatting(destfile)
        if 'vSummary' in destfile: trimvSum1(destfile); consol_vSum(destfile)
        else: print("SERVER WARNING: vSummary not found for final cleanup.")
        yield sse_message(" -> Final cleanup applied."); time.sleep(0.1)
        # --- REMOVED filter_rows() call from here ---
        # Save changes before summary generation
        ...
        # --- Step 13: Generate and Add Summary Sheets (Pandas) ---
        yield sse_message("Step 13: Generating analysis summaries..."); time.sleep(0.1)
        summary_sheets_added = False
        try:
            ...
            try:
                 ...
                 if 'Environment' in cols: cols.remove('Environment'); cols.insert(isfile_idx + 1, 'Environment') # Insert after Workload
            ...
            summary_overall_pon_counts = df_summary_source.groupby(['Workload', 'Environment', 'Powerstate'], observed=False).agg({'VM Count': 'sum'}).reset_index()
            ...
            summary_dc_pon_metrics = df_powered_on.groupby(['Datacenter', 'Cluster', 'Workload', 'Environment'], observed=False).agg(
                 PoweredOn_VMs=('VM Count', 'sum'), PoweredOn_Disks=('Disks', 'sum'), PoweredOn_Consumed_GB=('Consumed GB', 'sum')
            ).reset_index()
            ...
            # Write Summaries using append mode
            yield sse_message(" -> Writing summary sheets to Excel..."); time.sleep(0.1)
            with pd.ExcelWriter(output_filepath, mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
                ...
                summary_overall_total.to_excel(writer, sheet_name='Overall Summary Totals', index=False)
                summary_overall_pon_counts.to_excel(writer, sheet_name='Overall Powerstate Counts', index=False)
                summary_dc_combined.to_excel(writer, sheet_name='Datacenter Summary Combined', index=False)
            summary_sheets_added = True
            yield sse_message(" -> Summary sheets added."); time.sleep(0.1)

        except Exception as summary_err:
            print(f"SERVER ERROR: Generating/Adding summary sheets: {summary_err}")
            traceback.print_exc()
            yield sse_message(f"WARNING: Failed to add summary sheets: {summary_err}"); time.sleep(0.1)
            # Don't re-raise, allow process to continue for main file if possible

        # --- *** NEW Step 14: Reorder Sheets & Apply Filters (Openpyxl) *** ---
        yield sse_message("Step 14: Reordering sheets and applying filters..."); time.sleep(0.1)
        try:
            workbook = xl.load_workbook(output_filepath) # Load final workbook
            # Define desired order - put new sheets first
            desired_order = [
                'Overall Summary Totals',
                'Overall Powerstate Counts',
                'Datacenter Summary Combined',
                'vSummary', # Updated vSummary
                'vInfo',
                'vDisk',
                'vPartition'
            ]
            # Filter list to include only sheets that actually exist in the workbook
            current_sheets = workbook.sheetnames
            final_order = [sheet for sheet in desired_order if sheet in current_sheets]
            # Add any unexpected sheets (shouldn't be any) to the end
            for sheet in current_sheets:
                 if sheet not in final_order: final_order.append(sheet)
            ...
            # Apply filters to ALL sheets (including new ones)
            filter_rows(workbook) # Call the existing helper function
            yield sse_message(" -> Filters applied to all sheets."); time.sleep(0.1)

            # Save the reordered and filtered workbook
            workbook.save(output_filepath)
            yield sse_message(" -> Final workbook saved."); time.sleep(0.1)

        except Exception as final_step_err:
             print(f"SERVER ERROR: During final reorder/filter/save: {final_step_err}")
             traceback.print_exc()
