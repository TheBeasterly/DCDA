...
def view_results(client_name, project_name, filename_in_url):
    ...
    Determines if -ANALYZED and -EDITED versions exist for download and switch buttons.
    """
    ...
    safe_filename_in_url = secure_filename(filename_in_url) # File whose results are being viewed
    ...
    # Determine the true original base name from the filename_in_url
    current_file_base = ""
    filename_lower_in_url = safe_filename_in_url.lower()
    is_viewing_edited_version = False # Flag to indicate if the current view is for an -EDITED file
    ...
    if filename_lower_in_url.endswith("-analyzed.xlsx"):
        ...
        # This case implies it's neither -ANALYZED nor -EDITED.
        # For the purpose of finding related files, treat it as a base.
    ...
    logging.info(f"VIEW_RESULTS: Derived current_file_base: '{current_file_base}' from input '{safe_filename_in_url}'. Viewing edited: {is_viewing_edited_version}")

    # Define specific filenames for -ANALYZED and -EDITED versions using the derived base
    ...
    logging.info(f"VIEW_RESULTS: Constructed ANALYZED name: '{analyzed_version_name_on_disk}', EDITED name: '{edited_version_name_on_disk}'")
    ...
    if not os.path.isfile(path_to_file_being_viewed) or not os.path.abspath(path_to_file_being_viewed).startswith(analyzed_dir_abs):
        logging.error(f"VIEW_RESULTS: File to view not found or invalid path: {path_to_file_being_viewed}")
        flash(f"File '{html.escape(safe_filename_in_url)}' not found.", "error")
        return redirect(url_for('browse_existing'))
    logging.info(f"VIEW_RESULTS: Displaying data from: {path_to_file_being_viewed}")

    # Check for the presence of the standard -ANALYZED version
    ...
    logging.info(f"VIEW_RESULTS: Check for ANALYZED file '{path_to_analyzed_on_disk}': Exists = {analyzed_file_exists}")

    # Check for the presence of the -EDITED version
    ...
    logging.info(f"VIEW_RESULTS: Check for EDITED file '{path_to_edited_on_disk}': Exists = {edited_file_exists}")
    ...
    return render_template('results.html',
                           client_name=safe_client_name,
                           project_name=safe_project_name,
                           displayed_filename=safe_filename_in_url, # The file whose data is being shown
