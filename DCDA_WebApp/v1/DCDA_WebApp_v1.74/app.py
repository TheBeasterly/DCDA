...
@app.route('/upload', methods=['POST']) # Should already be POST only from original
def upload_file():
    """Handles AJAX file upload, checks collision, saves file, stores task, returns JSON."""
    
    # Default error response (in case of early exit)
    # HTTP 400 for Bad Request by client, 500 for Server Error
    
    if 'rvtools_file' not in request.files:
        logging.warning("Upload attempt with no file part.")
        return jsonify({"success": False, "message": "No file part selected."}), 400
    ...
    if not allowed_file(original_filename):
        logging.warning(f"Upload attempt with invalid file type: {original_filename}")
    ...
    client_name_to_use = ""
    if selected_client == '_new' and new_client_name:
        client_name_to_use = new_client_name
    elif selected_client and selected_client != '_new':
        client_name_to_use = selected_client
    else:
        logging.warning("Upload attempt without valid client selection.")
        return jsonify({"success": False, "message": "Please select an existing client or provide a name for a new client."}), 400

    safe_client_name = secure_filename(client_name_to_use)
    if not safe_client_name:
        logging.warning(f"Invalid client name provided for upload: '{client_name_to_use}'")
        return jsonify({"success": False, "message": f'Invalid client name provided: "{html.escape(client_name_to_use)}". Please use valid characters.'}), 400

    # --- Determine Project Name ---
    selected_project = request.form.get('selected_project')
    new_project_name = request.form.get('new_project_name', '').strip()
    project_name_to_use = DEFAULT_PROJECT_NAME 

    if selected_project == '_new' and new_project_name:
        project_name_to_use = new_project_name
    elif selected_project and selected_project not in ['_new', '', '_none_selected', '_general_select']:
        project_name_to_use = selected_project
    elif selected_client == '_new' or selected_project == DEFAULT_PROJECT_NAME or selected_project == '_general_select':
        project_name_to_use = DEFAULT_PROJECT_NAME

    safe_project_name = secure_filename(project_name_to_use)
    if not safe_project_name:
        safe_project_name = DEFAULT_PROJECT_NAME
        logging.warning(f"Project name '{project_name_to_use}' was invalid during upload, using default '{DEFAULT_PROJECT_NAME}'.")

    collision_action = request.form.get('collision_action', 'deny') # Default to 'deny' if not provided
    logging.info(f"Upload attempt (AJAX): Client='{safe_client_name}', Project='{safe_project_name}', File='{original_filename}', CollisionAction='{collision_action}'")

    try:
        client_project_path = os.path.join(app.config['DATA_FOLDER'], safe_client_name, safe_project_name)
        upload_dir = os.path.join(client_project_path, 'Originals')
        analyzed_dir = os.path.join(client_project_path, 'Analyzed')
        safe_original_filename = secure_filename(original_filename)
        input_filepath = os.path.join(upload_dir, safe_original_filename)
        analyzed_base, analyzed_ext = os.path.splitext(safe_original_filename)
        analyzed_filename_suffix = f"{analyzed_base}-ANALYZED{analyzed_ext}" # Keep consistent naming
        analyzed_filepath_check = os.path.join(analyzed_dir, analyzed_filename_suffix)

        os.makedirs(upload_dir, exist_ok=True)
        os.makedirs(analyzed_dir, exist_ok=True)
    except Exception as e:
        logging.error(f"Error constructing paths for upload (AJAX): {e}", exc_info=True)
        return jsonify({"success": False, "message": f"Server error creating directories: {e}"}), 500

    original_exists = os.path.exists(input_filepath) and os.path.isfile(input_filepath)
    analyzed_exists_check = os.path.exists(analyzed_filepath_check) and os.path.isfile(analyzed_filepath_check)
    collision_detected = original_exists or analyzed_exists_check

    if collision_detected and collision_action != 'overwrite':
        logging.warning(f"Upload cancelled (AJAX): Collision detected for '{safe_original_filename}' in {safe_client_name}/{safe_project_name} and overwrite denied.")
        return jsonify({"success": False, "message": f'File "{html.escape(original_filename)}" or its analyzed version already exists. Upload cancelled as overwrite was not selected.'}), 409 # 409 Conflict

    elif collision_detected and collision_action == 'overwrite':
        logging.warning(f"Upload Info (AJAX): Collision for '{safe_original_filename}' in {safe_client_name}/{safe_project_name}. Proceeding with OVERWRITE.")
        # Add logic here to delete existing files if overwrite means clean slate
        # For now, saving new file will just overwrite the original. The -ANALYZED might still exist if not re-processed.
    ...
    try:
        ...
        TASK_INFO[task_id] = {
            ...
            'output_folder': analyzed_dir,
            'client_name': safe_client_name,
            'project_name': safe_project_name,
            'timestamp': time.time()
        }
        logging.info(f"File '{safe_original_filename}' saved successfully (AJAX). Task {task_id} created for {safe_client_name}/{safe_project_name}.")
        
        processing_url = url_for('processing_page', task_id=task_id)
        return jsonify({
            "success": True,
            "message": f"File '{html.escape(safe_original_filename)}' uploaded successfully. Processing started...",
            "redirect_url": processing_url
        }), 200 # 200 OK or 201 Created or 202 Accepted
    
    except Exception as e:
        logging.error(f"Error saving file '{safe_original_filename}' or storing task (AJAX): {e}", exc_info=True)
...
def format_timestamp(ts):
    """Helper to format a timestamp into a readable string."""
    if ts is None:
        return None
    try:
...
def view_results(client_name, project_name, filename_in_url):
    # ... (existing setup and validation code) ...
    ...
    current_file_base = ""
    filename_lower_in_url = safe_filename_in_url.lower()
    is_viewing_edited_version = False

    if filename_lower_in_url.endswith("-analyzed.xlsx"):
        current_file_base = safe_filename_in_url[:-len("-analyzed.xlsx")]
    elif filename_lower_in_url.endswith("-edited.xlsx"):
        current_file_base = safe_filename_in_url[:-len("-edited.xlsx")]
        is_viewing_edited_version = True
    else:
        current_file_base, _ = os.path.splitext(safe_filename_in_url)
        logging.warning(f"VIEW_RESULTS: Filename '{safe_filename_in_url}' does not have a recognized suffix. Assuming base is '{current_file_base}'.")
    ...
    if not os.path.isfile(path_to_file_being_viewed) or not os.path.abspath(path_to_file_being_viewed).startswith(analyzed_dir_abs):
        flash(f"File '{html.escape(safe_filename_in_url)}' not found.", "error"); return redirect(url_for('browse_existing'))

    # Get timestamps
    analyzed_file_timestamp_str = None
    path_to_analyzed_on_disk = os.path.join(analyzed_dir_abs, analyzed_version_name_on_disk)
    analyzed_file_exists = os.path.isfile(path_to_analyzed_on_disk)
    if analyzed_file_exists:
        analyzed_file_timestamp_str = format_timestamp(os.path.getmtime(path_to_analyzed_on_disk))

    edited_file_timestamp_str = None
    path_to_edited_on_disk = os.path.join(analyzed_dir_abs, edited_version_name_on_disk)
    edited_file_exists = os.path.isfile(path_to_edited_on_disk)
    if edited_file_exists:
        edited_file_timestamp_str = format_timestamp(os.path.getmtime(path_to_edited_on_disk))
    
    # Read data from the file specified in the URL (path_to_file_being_viewed)
    try:
        ...
        logging.error(f"VIEW_RESULTS: Reading/converting Excel '{path_to_file_being_viewed}': {e}", exc_info=True)
    ...
    return render_template('results.html',
                           client_name=safe_client_name,
                           project_name=safe_project_name,
                           displayed_filename=safe_filename_in_url,
                           is_viewing_edited_version=is_viewing_edited_version,
                           analyzed_version_filename_for_actions=analyzed_version_name_on_disk if analyzed_file_exists else None,
                           analyzed_file_timestamp=analyzed_file_timestamp_str, # New
                           edited_version_filename_for_actions=edited_version_name_on_disk if edited_file_exists else None,
                           edited_file_timestamp=edited_file_timestamp_str,     # New
...
def get_files(client_name, project_name):
    """API endpoint called by JS to populate files for a selected project.
       Now includes timestamps for each file.
    """
    safe_client_name = secure_filename(client_name)
    safe_project_name = secure_filename(project_name)
    ...
    originals_list_with_ts = []
    processed_files_list_with_ts = []

    try:
        if os.path.isdir(originals_dir):
            items = os.listdir(originals_dir)
            for f_name in items:
                if os.path.isfile(os.path.join(originals_dir, f_name)) and not f_name.startswith('.'):
                    f_path = os.path.join(originals_dir, f_name)
                    timestamp = format_timestamp(os.path.getmtime(f_path))
                    originals_list_with_ts.append({"name": f_name, "timestamp": timestamp})
            originals_list_with_ts.sort(key=lambda x: x['name'].lower())
    except Exception as e:
    ...
    try:
        if os.path.isdir(analyzed_dir):
            ...
            for f_name in items:
                if os.path.isfile(os.path.join(analyzed_dir, f_name)) and \
                   not f_name.startswith('.') and \
                   (f_name.lower().endswith('-analyzed.xlsx') or f_name.lower().endswith('-edited.xlsx')):
                    f_path = os.path.join(analyzed_dir, f_name)
                    timestamp = format_timestamp(os.path.getmtime(f_path))
                    processed_files_list_with_ts.append({"name": f_name, "timestamp": timestamp})
            processed_files_list_with_ts.sort(key=lambda x: x['name'].lower())
    except Exception as e:
        logging.error(f"API Error scanning Processed files for {safe_client_name}/{safe_project_name}: {e}", exc_info=True)

    logging.info(f"API get_files for {safe_client_name}/{safe_project_name}: Found {len(originals_list_with_ts)} Originals, {len(processed_files_list_with_ts)} Processed.")
    return jsonify({"originals": originals_list_with_ts, "analyzed": processed_files_list_with_ts})
...
# --- Delete File Route ---
# ================================================================
@app.route('/delete_file/<client>/<project>/<file_type>/<path:filename>', methods=['DELETE'])
def delete_file_route(client, project, file_type, filename):
    """Deletes a specified file (Original, Analyzed, or Edited)."""
    safe_client = secure_filename(client)
    safe_project = secure_filename(project)
    safe_filename = secure_filename(filename) # Filename from URL path part
    
    log_prefix = f"Delete Request [{safe_client}/{safe_project}/{file_type}/{safe_filename}]:"
    logging.info(f"{log_prefix} Received.")

    if not safe_client or not safe_project or not safe_filename:
        logging.error(f"{log_prefix} Invalid client, project, or filename in request.")
        return jsonify({"success": False, "message": "Invalid client, project, or filename."}), 400

    if file_type not in ['Originals', 'Analyzed']:
        logging.error(f"{log_prefix} Invalid file type specified: {file_type}")
        return jsonify({"success": False, "message": "Invalid file type specified."}), 400

    # Construct the full path to the file
    # The 'Analyzed' folder contains both -ANALYZED.xlsx and -EDITED.xlsx files
    target_folder_abs = os.path.abspath(os.path.join(app.config['DATA_FOLDER'], safe_client, safe_project, file_type))
    filepath_to_delete = os.path.normpath(os.path.join(target_folder_abs, safe_filename))
    
    # Security check: ensure final path is within the designated data folder structure
    # Check against the specific subfolder ('Originals' or 'Analyzed')
    if not os.path.abspath(filepath_to_delete).startswith(target_folder_abs):
         logging.error(f"{log_prefix} Path traversal attempt detected. Path: '{filepath_to_delete}' is outside '{target_folder_abs}'")
         return jsonify({"success": False, "message": "Invalid file path (security)."}), 403

    try:
        if os.path.isfile(filepath_to_delete):
            os.remove(filepath_to_delete)
            logging.info(f"{log_prefix} Successfully deleted file: {filepath_to_delete}")
            return jsonify({"success": True, "message": f"File '{html.escape(safe_filename)}' deleted successfully."})
        else:
            logging.warning(f"{log_prefix} File not found for deletion: {filepath_to_delete}")
            return jsonify({"success": False, "message": f"File '{html.escape(safe_filename)}' not found."}), 404
    except Exception as e:
        logging.error(f"{log_prefix} Error deleting file {filepath_to_delete}: {e}", exc_info=True)
